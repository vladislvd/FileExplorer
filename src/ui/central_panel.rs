...
use crate::services::{draw_item, rename_operation_window, paste_operation};
...
fn draw_files(
    ...
    egui::ScrollArea::vertical()
        ...
        .show_rows(ui, row_height, app.visible_files.len(), |ui, rows|{
...
fn right_click_menu(
    ...
    response.context_menu(|ui| {
        ...
        if ui.button(format!("{} New file", icons::ICON_NOTE_ADD)).clicked(){

            ui.close();
        }

        if ui.button(format!("{} New folder", icons::ICON_CREATE_NEW_FOLDER)).clicked(){
            println!("New dir");
            ui.close();
        }
        ...
        if ui.add_enabled(is_clipboard_empty, egui::Button::new(format!("{} Paste", icons::ICON_CONTENT_PASTE))).clicked(){
            paste_operation(app);
            ui.close();
        }
    });
